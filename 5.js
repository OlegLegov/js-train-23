...
class BankTransfer {
  ...
  //  Метод initiateTransfer, який приймає amount та відповідає за ініціювання банківського переказу.
  ...
  // Метод calculateFee, який відповідає за розрахунок комісії за переказ.
  calculateFee(amount) {
...
class WalletTransfer {
  ...
  // Метод processTransfer, який відповідає за здійснення переказу з гаманця.
...
class TransferAdapter {
  ...
  // Метод initiateTransfer, який адаптує API WalletTransfer до API BankTransfer.
  initiateTransfer(amount) {
    const calculatedAmount = this.calculateFee(amount);
    return this.transferSystem.processTransfer(calculatedAmount);
  }

  // Метод calculateFee, що приймає amount та обчислює суму комісії за переказ.
  calculateFee(amount) {
