...
class Basket {
  ...
  // Метод addGood, що приймає один параметр - good, який потрібно додати до масиву
  addGood(good) {
    // Додаємо новий товар в масив товарів
    this.goods.push(good);
  }

  // Метод calculateTotalPrice, що розраховує загальну вартість товарів в кошику з урахуванням знижки
  calculateTotalPrice() {
    // Сумуємо вартість всіх товарів в масиві за допомогою метода reduce
    let totalPrice = this.goods.reduce((total, good) => total + good.price, 0);
    // Застосовуємо знижку до загальної вартості за допомогою метода applyDiscount нашого об'єкта discountPlan
...
class RegularDiscountPlan extends Basket {
  ...
  // Метод applyDiscount, що приймає ціну price як параметр
  applyDiscount(price) {
    // Повертає ціну з урахуванням знижки в 10%
    return price * 0.9;
  }
}
...
class VIPDiscountPlan extends Basket {
  ...
  // Метод applyDiscount, що приймає ціну price як параметр
  applyDiscount(price) {
    // Повертає ціну з урахуванням знижки в 20%
    return price * 0.8;
  }
}
...
class NewClientDiscountPlan extends Basket {
  ...
  // Метод applyDiscount, що приймає ціну price як параметр
  applyDiscount(price) {
    // Повертає ціну з урахуванням знижки в 5%
    return price * 0.95;
  }
}
